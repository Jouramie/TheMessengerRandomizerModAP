...
public class ArchipelagoMenu
{
    ...
    public static void DisplayArchipelagoMenu()
    {
        ...
        if (!ArchipelagoScreen.RandoScreenLoaded)
            archipelagoScreen = ArchipelagoScreen.BuildModOptionScreen(
                Manager<UIManager>.Instance.GetView<OptionScreen>()
            );
    ...
    public class ArchipelagoScreen : ModOptionScreen
    {
        ...
        public static new ArchipelagoScreen BuildModOptionScreen(OptionScreen optionScreen)
        {
            ...
            archipelagoScreen.optionMenuButtons = archipelagoScreen
                .transform.Find("Container")
                .Find("BackgroundFrame")
                .Find("OptionsFrame")
                .Find("OptionMenuButtons");
        ...
        public new void BackToOptionMenu()
        {
            ...
            ArchipelagoMenuButton.gameObject.transform.Find("Button").GetComponent<UIObjectAudioHandler>().playAudio =
                false;
            EventSystem.current.SetSelectedGameObject(
                ArchipelagoMenuButton.gameObject.transform.Find("Button").gameObject
            );
            ArchipelagoMenuButton.gameObject.transform.Find("Button").GetComponent<UIObjectAudioHandler>().playAudio =
                true;
        }
    ...
    {
        ArchipelagoMenuButton = Courier.UI.RegisterSubMenuOptionButton(
            () => "Archipelago Randomizer",
            DisplayArchipelagoMenu
        );

        // These should always be visible
        VersionButton = RegisterSubRandoButton(
        ...
        ArchipelagoNameButton = RegisterTextRandoButton(
            () => "Enter Archipelago Slot Name",
            APRandomizerMain.OnSelectArchipelagoName,
            16,
            () => "Enter player name:",
        ...
        SendChatMessageButton = RegisterTextRandoButton(
            ...
            charset: TextEntryButtonInfo.CharsetFlags.Letter
                | TextEntryButtonInfo.CharsetFlags.Number
                | TextEntryButtonInfo.CharsetFlags.Space
                | TextEntryButtonInfo.CharsetFlags.Dash
                | TextEntryButtonInfo.CharsetFlags.Dot
        ...
        ArchipelagoStatusButton = RegisterSubRandoButton(
            () => ArchipelagoClient.DisplayStatus ? "Hide status information" : "Display status information",
        ...
        ArchipelagoToggleMessagesButton = RegisterSubRandoButton(
            () => ArchipelagoClient.DisplayAPMessages ? "Hide server messages" : "Display server messages",
        ...
        ShuffleMusicButton = RegisterSubRandoButton(
            () => RandoMusicManager.ShuffleMusic ? "Disable Music Shuffle" : "Enable Music Shuffle",
        ...
        ArchipelagoDeathLinkButton = RegisterSubRandoButton(
            () => ArchipelagoData.DeathLink ? "Disable Death Link" : "Enable Death Link",
